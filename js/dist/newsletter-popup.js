(function(){"use strict";const i="mh_newsletter_dismissed";let a=!1;function d(){const t=localStorage.getItem(i);if(!t)return!0;const e=parseInt(t,10);return(Date.now()-e)/(1e3*60*60*24)>=7}function r(){localStorage.setItem(i,Date.now().toString());const t=document.getElementById("mh-newsletter-popup");t&&(t.style.opacity="0",t.style.transform="translate(-50%, -50%) scale(0.95)",setTimeout(()=>t.remove(),400));const e=document.getElementById("mh-popup-overlay");e&&(e.style.opacity="0",setTimeout(()=>e.remove(),400))}async function c(t){const e=document.getElementById("mh-popup-submit"),o=document.getElementById("mh-popup-msg");e.disabled=!0,e.textContent="P\u0159ihla\u0161uji...";try{const n=window.API_CONFIG?.BASE_URL||"/api",f=await fetch(`${n}/csrf-token`,{credentials:"include"}),{csrfToken:_t,token:_k}=await f.json(),csrf=_t||_k||"",p=await(await fetch(`${n}/newsletter/subscribe`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json","X-CSRF-Token":csrf},body:JSON.stringify({email:t,source:"web_popup"})})).json();p.success?(o.textContent="\u{1F31F} Skv\u011Bl\xE9! Brzy v\xE1m p\u0159ijde prvn\xED hv\u011Bzdn\xE1 zpr\xE1va.",o.style.color="#4ade80",setTimeout(r,2500)):(o.textContent=p.error||"Chyba. Zkuste to znovu.",o.style.color="#f87171",e.disabled=!1,e.textContent="Odeb\xEDrat zdarma")}catch{o.textContent="Chyba p\u0159ipojen\xED. Zkuste to znovu.",o.style.color="#f87171",e.disabled=!1,e.textContent="Odeb\xEDrat zdarma"}}function s(){if(a||!d())return;a=!0;const t=document.createElement("div");t.id="mh-popup-overlay",t.style.cssText=`
            position:fixed; inset:0; background:rgba(0,0,0,0.65);
            z-index:9998; opacity:0; transition:opacity 0.4s ease;
            backdrop-filter:blur(4px);
        `,t.addEventListener("click",r);const e=document.createElement("div");e.id="mh-newsletter-popup",e.setAttribute("role","dialog"),e.setAttribute("aria-label","P\u0159ihl\xE1sit se k odb\u011Bru"),e.style.cssText=`
        ...
        `,document.body.appendChild(t),document.body.appendChild(e),requestAnimationFrame(()=>{requestAnimationFrame(()=>{t.style.opacity="1",e.style.opacity="1",e.style.transform="translate(-50%, -50%) scale(1)"})}),document.getElementById("mh-popup-close").addEventListener("click",r),document.getElementById("mh-popup-submit").addEventListener("click",()=>{const n=document.getElementById("mh-popup-email").value.trim();if(!n){document.getElementById("mh-popup-msg").textContent="Zadejte pros\xEDm emailovou adresu.",document.getElementById("mh-popup-msg").style.color="#f87171";return}c(n)}),document.getElementById("mh-popup-email").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("mh-popup-submit").click()}),e.querySelector("#mh-popup-email").focus()}function l(){if(!d())return;document.addEventListener("mouseleave",e=>{e.clientY<=0&&s()},{passive:!0}),setTimeout(s,45e3);let t=!1;window.addEventListener("scroll",()=>{if(t)return;window.scrollY/(document.body.scrollHeight-window.innerHeight)>.7&&(t=!0,setTimeout(s,2e3))},{passive:!0})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",l):l()})();
