(function(){"use strict";const a="mh_visit_count",n="mh_push_subscribed";if(!("serviceWorker"in navigator)||!("PushManager"in window))return;function u(){return parseInt(localStorage.getItem(a)||"0",10)}function m(){const t=u()+1;return localStorage.setItem(a,t),t}async function c(){try{if(await Notification.requestPermission()!=="granted")return localStorage.setItem(n,"denied"),!1;const e=await navigator.serviceWorker.ready;let o;try{o=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:f(window.VAPID_PUBLIC_KEY||"BEl62iUYgUivxIkv69yViEuiBIa-Ib9-SkvMeAtA3LFgDzkrxZJjSgSnfckjBJuBkr3qBrqLHy9-Ndgo292mkiw")})}catch{return localStorage.setItem(n,"intent"),!0}const r=window.API_CONFIG?.BASE_URL||"/api",i=window.getCSRFToken?await window.getCSRFToken():null;return await fetch(`${r}/push/subscribe`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json",...i&&{"X-CSRF-Token":i}},body:JSON.stringify({subscription:o})}),localStorage.setItem(n,"active"),!0}catch(t){return console.warn("[Push] Subscription failed:",t),!1}}function f(t){const e="=".repeat((4-t.length%4)%4),o=(t+e).replace(/-/g,"+").replace(/_/g,"/"),r=atob(o);return new Uint8Array([...r].map(i=>i.charCodeAt(0)))}function d(){const t=localStorage.getItem(n);if(t==="active"||t==="denied")return;const e=document.createElement("div");e.id="mh-push-banner",e.style.cssText=`
        ...
        `;const i=document.createElement("style");i.textContent="@keyframes slideUp{from{opacity:0;transform:translateX(-50%) translateY(20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}",document.head.appendChild(i),document.body.appendChild(e),document.getElementById("mh-push-yes").addEventListener("click",async()=>{if(e.remove(),await c()){const s=document.createElement("div");s.style.cssText="position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);z-index:9999;background:#166534;color:#fff;padding:0.75rem 1.5rem;border-radius:50px;font-size:0.9rem;font-family:'Inter',sans-serif;",s.textContent="\u{1F514} Notifikace zapnuty! Uvid\xEDme se z\xEDtra r\xE1no.",document.body.appendChild(s),setTimeout(()=>s.remove(),3e3)}}),document.getElementById("mh-push-no").addEventListener("click",()=>{localStorage.setItem(n,"denied"),e.remove()}),setTimeout(()=>e?.remove(),12e3)}function l(){const t=m(),e=document.getElementById("subscribe-push-btn");if(e&&(localStorage.getItem(n)==="active"&&(e.innerHTML="\u{1F515} Zru\u0161it odb\u011Br horoskopu",e.classList.add("btn--active")),e.addEventListener("click",async()=>{localStorage.getItem(n)==="active"?(localStorage.removeItem(n),e.innerHTML="\u{1F514} Odeb\xEDrat denn\xED horoskop",e.classList.remove("btn--active"),window.Auth?.showToast&&window.Auth.showToast("Info","Odb\u011Br horoskopu byl zru\u0161en.","info")):await c()&&(e.innerHTML="\u{1F515} Zru\u0161it odb\u011Br horoskopu",e.classList.add("btn--active"),window.Auth?.showToast&&window.Auth.showToast("\xDAsp\u011Bch","Odb\u011Br horoskopu byl aktivov\xE1n.","success"))})),t>=2)if(localStorage.getItem("cookieConsent"))setTimeout(d,5e3);else{const r=setInterval(()=>{localStorage.getItem("cookieConsent")&&(clearInterval(r),setTimeout(d,3e3))},500)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",l,{once:!0}):l()})();
