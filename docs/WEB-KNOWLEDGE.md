...
| | |
|---|---|
| **Datum analýzy** | 2026-07-12, revize 2026-07-29 |
| **Commit** | `3d1d2518` (revize; původní analýza `89fa8982`) |
| **HTML stránek** | 923 (907 v sitemap.xml) |
...
>
> **Revize 2026-07-29:** opraveny fakticky špatné údaje (URL Stripe webhooku), doplněn stav VAPID a e-mailového mostu. Změny od `89fa8982`: CTR přepis title/description ~230 stránek (tarot významy 78 + partnerská shoda 144), e-mailový most `js/horoscope-subscribe.js` na třech plochách, oprava cache-key v denním horoskopovém e-mailu (šetří placené AI volání), viditelné logování odmítnutých funnel eventů, deaktivace trvale neplatných příjemců v e-mailové frontě.

---

## 1. Přehled

Česká ezoterická platforma (astrologie, tarot, numerologie, runy…) monetizovaná předplatným přes Stripe + jednorázovými PDF produkty. Stack: **statické HTML + vanilla JS** (IIFE bundly přes esbuild do `js/dist/`), **Express server** (`server/index.js`, 1217 ř.), **Supabase** (DB), **Railway** (deploy, za Cloudflare), **Resend** (e-maily), **Anthropic API** (AI interpretace), **node-schedule** (cron joby), **web-push**. Doména `https://www.mystickahvezda.cz`.

- North star: `weekly_ritual_to_paid_intent` — uživatel dokončí osobní rituál v týdnu a vytvoří placený intent (`server/config/growth-loop.js:24`).
...
| Blog | 74 | `blog/` | `templates/blog-post.html` + `npm run blog:cluster-links` | 1–2 posty/týden dle seo-growth-plan |
...
cenik.html  … js/cenik.js čte GET /api/plans (manifest z constants.js) … pricing_plan_cta_clicked
  ...
  ▼
Stripe → webhook POST /webhook/stripe  +  fallback: success page → dashboard.js pošle session_id
       → jednorázové PDF: reconciliation job (verify přes Stripe API před fulfillmentem)
```

Runtime ověřeno (server v bezpečném e2e režimu, viz §7): stránky funnelu 200; registrace → onboarding → profil funguje včetně cookies; `POST /api/payment/funnel-event` přijímá **`eventName`** (ne eventType — route `server/payment.js:978`, zápis `recordFunnelEvent` na `:576`); `POST /api/analytics/event` přijímá; redirecty 301 fungují.

**⚠️ Platby — kritický kontext:**
...
**Známé mezery měření:** klientské eventy (vč. `purchase_completed` v GA4) jsou consent-gated → **systematicky podhodnocené**; server-side purchase eventy závisí na doručení webhooku (§3). Pravda o výnosech = `npm run monitor:revenue-truth:production` + Stripe API, ne GA4. Interpretační pravidla propadů: operator-context „Decision Rules".

⚠️ **Tichá selhání zápisu (opraveno 2026-07-25, `78f30019` + `2f9a9cd1` — ale vzor si pamatuj):** Supabase JS klient u chyby DB **nevyhazuje výjimku** — vrací `{ data, error }`. `recordFunnelEvent` (`server/payment.js`) proto dřív zahazoval odmítnuté inserty bez jediného logu a data mizela potichu; dnes chybu loguje včetně `code`/`details`/`hint`. Stejná past platí všude, kde se píše do DB — **vždy testuj `error`, ne jen `try/catch`**. Druhý zdroj slepoty: bez CSRF tokenu `js/analytics.js` i `js/premium-gates.js` zahazují eventy tichým `return`, takže výpadek `/api/csrf-token` vypne veškeré měření naráz.
...
**Testy:** Jest — 61 server specs (`server/tests/`) + 4 frontend (ve worktree nutný explicitní `--testMatch`, lessons.md); Playwright — 27 e2e specs v sekcích api/core/content/tools/checkout (`scripts/run-e2e-sections.mjs`), smoke skripty pro funnel. Brána před pushem: `npm run test:verify` (lint, encoding, hooks, auth-contexts, ai-cost, growth-loop, paywall-trust, unit, audit:site, tarot-assets, sitemap:check).

## 6. SEO infrastruktura

- **Sitemap:** `sitemap.xml` 907 URL, generovaná z canonicalů (`npm run sitemap:generate`, kontrola `sitemap:check`); dynamická `horoskop/sitemap-horoscopes.xml`. Obě v `robots.txt`.
...
- **Redirect vrstvy** (`server/index.js:742-800`, runtime ověřeno): `STATIC_PAGE_REDIRECTS` (diakritika, přejmenované stránky), `kompatibilita/{cz}-{cz}.html` → 301 `partnerska-shoda/{en}-{en}.html` (formát `beran-byk`, ne `beran-a-byk`), `/jmena/{Jméno}` → 301 lowercase `.html`.
...
2. ~~Auth handoff leak (91% ztráta)~~ — **opraveno, ověřeno živými daty 2026-07-12** (auth page 21 → form submit 18 = 94 %; checkout_requested 27 → checkout_started 27 = 100 %).
   ...
   **Strukturální poznatek 2026-07-25 (drž se ho, čísla stárnou):** konverze paywallu se **nesmí měřit průměrem přes všechny plochy** — plochy se liší o dva řády podle toho, v jakém stavu uživatele zastihnou.
   - Paywall po **dokončeném** zážitku (`tarot_yes_no_result` — uživatel dostal svou odpověď ano/ne) konvertuje **prakticky na nule**. Není to chyba copy ani ceny: potřeba je uspokojená, není co dokupovat.
   - Paywall **uprostřed** rozdělané potřeby (`tarot_auth_gate`) konvertuje řádově líp.
   - Proto byl na tarot ano/ne paywall **nahrazen sběrem e-mailů** (`c46943c5`) — z hotového zážitku se dá vzít kontakt, ne platba.

   **Skutečný strop je objem, ne míra konverze** — placených zákazníků jsou jednotky, takže každé procento počítané z takhle malého jmenovatele je šum. Nedělej z něj závěry o copy. Živá čísla ber z `GET /api/admin/funnel` / `npm run analyze:funnel`, ne odsud.

   ⚠️ Do funnelu se počítá i **vlastní testovací provoz** — vyloučení řeší `MH_FUNNEL_EXCLUDED_USER_IDS`.
...
6. ~~**Push notifikace v produkci potvrzeně mrtvé**~~ — **opraveno 2026-07-29.** `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` doplněny do Railway env; `/api/config` vrací `vapidPublicKey` a `features.pushNotifications: true`, `/api/health` hlásí `pushNotifications: "configured"`. **Zbývá dosbírat publikum** — klient dřív bez public key nemohl ukládat subscriptions, takže `push_subscriptions` startuje prakticky od nuly; první dny bude denní push job posílat málokomu. `sentryDsn` je v produkci **stále `null`** (jediná zbývající chybějící env proměnná z tohoto bodu).
7. **Alerting je hotový v kódu, ale nejspíš „potmě"** (přepsáno 2026-07-29 — dřív tu stálo „nedodělaný", což je nepřesné). `server/services/alerts.js` umí kritické alerty na `stripe_webhook_failed` a `checkout_session_failed` (`server/payment.js:382`) i na 5xx spiky, včetně deduplikace a rate limitu. **Jenže `enabled` = `Boolean(webhookUrl)`** (`alerts.js:104`) → bez `OPERATIONAL_ALERT_WEBHOOK_URL` (nebo `ALERT_WEBHOOK_URL`) v Railway env se alert **tiše zahodí**. Než se bude spoléhat na „nic nepřišlo = nic nehoří", ověřit, že je proměnná nastavená. Zbývá dál: Lighthouse/CWV pass na produkci (app-improvement-plan Tier 3).
...
| Debug plateb | Webhook = **`POST /webhook/stripe`** (mimo `/api`, raw body) → §3; pak Stripe API live (`checkout.sessions.list`) + `payment_events` + delivery log endpointu; local klíč = TEST, produkce LIVE |
| Debug měření | §4 „Tichá selhání zápisu" — nejdřív ověř, že event vůbec dojde (CSRF token, `error` z Supabase), teprve pak řeš copy |
...
Při další velké analýze: `git diff 3d1d2518..HEAD --stat` → aktualizovat jen dotčené sekce → přepsat hlavičku (datum, commit, počty stránek: `find . -name "*.html" -not -path "./node_modules/*" -not -path "./docs/*" | wc -l` a `grep -c "<loc>" sitemap.xml`). Plný re-run není potřeba, dokud se nemění architektura (server framework, build, DB).

**Pravidlo pro zápis (draho zaplaceno 2026-07-29):** tenhle dokument se čte jako pravda, takže **nepiš sem nic, co jsi neověřil**. URL webhooku tu byla přes dva týdny špatně (`/api/payment/webhook` místo `/webhook/stripe`) a poslala celou diagnózu plateb špatným směrem — včetně závěru „webhooky nechodí, root cause je Stripe Dashboard", který se ukázal jako nesprávný. Když fakt neznáš, napiš **„neověřeno"**; to je užitečnější než odhad, který vypadá jako zjištění. Konkrétně: cesty a názvy eventů ověřuj grepem, produkční stav endpointem (`/api/health`, `/api/config`), čísla funnelu dotazem — a ke každému tvrzení připiš datum, aby šlo poznat, co mezitím zestárlo.
